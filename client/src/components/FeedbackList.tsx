      ...
      {isLoading ? (
        ...
        <div className="animate-pulse">
          {[0, 1].map((i) => (
            <div
              key={i}
              className="px-4 py-3.5 border-b border-surface-code last:border-b-0"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="h-4 w-16 bg-surface-code rounded-full" />
                <div className="h-3 w-10 bg-surface-code rounded" />
              </div>
              <div className="h-3.5 w-3/4 bg-surface-code rounded" />
            </div>
          ))}
        </div>
      ) : items.length === 0 ? (
        ...
        <div className="flex flex-col items-center px-5 py-8 text-center">
          <span className="w-11 h-11 rounded-xl bg-surface-code text-text-caption flex items-center justify-center mb-2.5">
            <Edit3 size={18} />
          </span>
        ...
        // 목록 — 각 item은 부모에서 stagger 클래스를 생성해 전달 (순차 페이드인)
        items.map((item, idx) => (
          <FeedbackItem
            key={item.feedbackUuid}
            item={item}
            className={stagger(idx).className}
          />
        ))
      )}
    </div>
