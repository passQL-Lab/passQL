...
- **daisyUI 컴포넌트를 최우선으로 사용한다** — 버튼(`btn`), 모달(`modal`), 로딩(`loading`), 배지(`badge`), 인풋(`input`), 카드(`card`) 등 daisyUI에 있는 컴포넌트는 반드시 daisyUI 클래스를 기반으로 구현한다
- Tailwind 유틸리티는 daisyUI로 커버되지 않는 세부 조정(색상, 간격, 크기 등)에만 보조적으로 사용한다
- 버튼을 만들 때 `<button className="btn ...">` 형태가 기본. 순수 Tailwind로 버튼을 처음부터 만들지 않는다
...
커밋 메시지는 반드시 아래 형식을 따른다 (`/commit` 스킬 컨벤션과 동일):

```
{이슈제목} : {타입} : {변경사항 설명} {이슈URL}
```

**이슈제목 형식**: `[이모지+태그][카테고리] 제목`
...
**타입 목록**: `feat` / `fix` / `refactor` / `docs` / `chore` / `style` / `test`

**규칙**:
- 이모지와 `[` 사이 공백 없음: `⚙️[기능추가]` (O), `⚙️ [기능추가]` (X)
- 허용 목록 외 이모지 사용 금지
- 이슈 URL은 반드시 끝에 포함
- 커밋 전 반드시 사용자에게 메시지 확인받을 것

**예시**:
```
⚙️[기능추가][로그인] 소셜 로그인 버튼 추가 : feat : Google/Kakao/Naver/GitHub/Apple 버튼 구현 https://github.com/passQL-Lab/passQL/issues/277
❗[버그][로그인] 레이아웃 파괴 버그 : fix : tokens.css spacing 변수 충돌 해결 https://github.com/passQL-Lab/passQL/issues/277
```
