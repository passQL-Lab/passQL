...
- **questionUuid** (선택): 있으면 기존 문제 업데이트, 없거나 null이면 신규 등록. 제거하면 복제/이관 용도로 활용 가능
...
- **choiceSetPolicy** (선택): 위 목록에서 선택. 생략 시 `AI_ONLY`로 기본 적용
  - `ODD_ONE_OUT`: "다음 중 SQL 실행 결과가 다른 것은?" 유형에 사용. 내보내기 시 DB에 저장된 값이 그대로 출력됨
...
- **schemaSampleData** (선택): INSERT 문 목록. 문제에 데이터가 없으면 실행 가능한 최소 데이터 유추 생성. CONCEPT_ONLY면 null
- **schemaIntent** (선택): 이 테이블/데이터가 어떤 개념을 테스트하는지 한 줄 설명. CONCEPT_ONLY면 null
- **answerSql** (EXECUTABLE 필수): 정답 번호에 해당하는 SELECT 쿼리. CONCEPT_ONLY면 null
  - `ODD_ONE_OUT` 유형: 나머지 3개와 다른 결과를 내는 선택지의 SQL
...
- `questionUuid`를 제거하면 **복제/이관** 용도로 활용 가능
...
아래 프롬프트를 AI에게 전달하고, 문제 데이터를 붙여넣으면 JSON 배열로 변환해줍니다.

```
아래 SQLD 문제 데이터를 passQL 대량 JSON 명세로 변환해줘.

출력은 JSON 배열 코드블록 하나만. 다른 설명 없이.
